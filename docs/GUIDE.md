# Lodestone Transit — player guide
...
- [Navigation and portable use](#navigation-and-portable-use)
- [Recipes](#recipes)
- [Physical anchors and names](#physical-anchors-and-names)
- [Arrival, mounts and leads](#arrival-mounts-and-leads)
- [Optional compatibility](#optional-compatibility)
- [Develop in IntelliJ IDEA](#develop-in-intellij-idea)
...
![Fully charged stations surrounding a Calibrated Lodestone, viewed from above](images/calibrated-lodestone-and-stations.png)
...
The hotbar shade and crosshair indicator share the server's per-hand readiness state. Failure messages appear briefly in the action bar, not chat, and distinguish the reason (empty fuel, warming up, invalid or uncalibrated anchor, moving anchor, missing death, unavailable or wrong dimension, unsafe arrival, and passenger restrictions). [Full message list](ACTION-BAR-MESSAGES.md).
...
![Dimensional Core: the hand-painted Fractured Eye sprite](images/dimensional-core.png)

An Eye of Ender split by a dark dimensional fissure and held by three ivory Nether Star fragments. The native texture is 16 × 16; the preview above is enlarged without smoothing. [See the Core in-game](images/dimensional-core-in-game.png).
...
![Inventory showing the five charge states of both portable teleporters](images/teleporter-charge-states.png)
...
![Normal Teleporter held in first person with the optional 3D resource packs enabled](images/teleporter-3d.png)

The Dimensional Teleporter uses an ivory frame. Both retain the round inventory icon:

![Dimensional Teleporter held in first person with the optional 3D resource packs enabled](images/dimensional-teleporter-3d.png)

See [verification and playtesting](TESTING.md) for the exact tested versions and remaining human checks.
...
![Fully charged stations illuminating the stone platform at midnight](images/station-light-night.png)
...
Architecture and mixin boundaries are documented in [ARCHITECTURE.md](ARCHITECTURE.md). English and Spanish (Spain) translations are included. The repository's original [GPLv3 license](../LICENSE) is retained.

Release changes and update instructions are in [CHANGELOG.md](../CHANGELOG.md). Substantial code, text and artwork were produced by an AI agent under human direction; see [provenance and credits](PROVENANCE.md).
