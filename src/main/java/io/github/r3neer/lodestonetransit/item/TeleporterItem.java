...
public final class TeleporterItem extends Item {
    ...
    @Override public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!EquipReadiness.ready(serverPlayer, hand)) return InteractionResult.FAIL;
        int fuel = stack.getOrDefault(LodestoneTransit.CHARGES, 0);
        if (fuel <= 0) { TeleportService.feedback(serverPlayer, false); return InteractionResult.FAIL; }
        if (!EquipReadiness.claimAttempt(serverPlayer)) return InteractionResult.FAIL;
    ...
    @Override public void inventoryTick(ItemStack stack, ServerLevel level, Entity owner, EquipmentSlot slot) { CompassAnchors.update(stack, level); }
